...
The problem dir is expected to contain ``submission/problem.{txt,tex,md}``
(or a single ``.txt/.tex/.md`` file in ``submission/``). The script runs the
...
def _update_metadata(
    metadata_path: Path, verdict: dict[str, Any], model_spec: str
) -> None:
...
async def amain() -> int:
    ...
    if metadata_path.exists():
        _update_metadata(metadata_path, verdict_dict, model_spec)
